...
class CameraSensorTrackingControl(Node):
    def __init__(self):
        ...
        self.declare_parameter('left_search_min', 0.05, range_desc)
        self.declare_parameter('left_search_max', 0.45, range_desc)
        self.declare_parameter('right_search_min', 0.55, range_desc)
        self.declare_parameter('right_search_max', 0.95, range_desc)
        ...
        self.nwindows = 9         
        self.margin = 80          # [수정] 커브 대응을 위해 margin 확장 (50 -> 80)
        self.minpix = 20          # [수정] 픽셀 조건 완화

        # 이전 프레임 정보 및 차선 폭 저장 변수
        self.left_fit = None
        self.right_fit = None
    ...
    def binarize_lane(self, bgr_frame):
        ...
        scaled_sobel = np.uint8(255 * abs_sobelx / max_sobel) if max_sobel > 0 else np.uint8(abs_sobelx)
        ...
        return cv2.morphologyEx(combined_binary, cv2.MORPH_CLOSE, kernel)
    ...
    def fit_polynomial_sliding_window(self, binary_warped):
        ...
        left_fit = np.polyfit(lefty, leftx, 2) if len(leftx) > 150 else None
        right_fit = np.polyfit(righty, rightx, 2) if len(rightx) > 150 else None
    ...
    def image_callback(self, msg):
        ...
        # 이전 곡선 계수가 모두 존재할 경우 Margin Search 수행
