...
class CameraSensorTrackingControl(Node):
    ...
    def sanity_check(self, left_fit, right_fit):
        ...
        # 1. 곡률 유사성 검증 (2차항 계수 A의 차이가 심하면 왜곡으로 판단)
        A_diff = abs(left_fit[0] - right_fit[0])
        if A_diff > 0.0015:
            return False

        # 2. 도로 폭 검증 (상단 및 하단에서의 좌우 차선 간격)
        ...
        # 기준 차선 폭 대비 허용 범위를 벗어나면 검증 실패
        min_w = self.lane_width * 0.70
        max_w = self.lane_width * 1.30

        if not (min_w < bottom_width < max_w) or not (min_w < top_width < max_w):
            return False
